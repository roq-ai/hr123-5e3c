...
export function TeamListPage(props: TeamListPageProps) {
  ...
  const columns: ColumnType[] = [
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_team_team_lead_idTouser',
          header: 'User Team Team Lead Id Touser',
          accessorKey: 'user_team_team_lead_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_team_team_lead_idTouser?.id}`}
            >
              {record.user_team_team_lead_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_team_member_idTouser',
          header: 'User Team Member Id Touser',
          accessorKey: 'user_team_member_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_team_member_idTouser?.id}`}
            >
              {record.user_team_member_idTouser?.email}
            </Link>
          ),
        }
      : null,
